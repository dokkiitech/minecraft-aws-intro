...
バックアップは放っておくと無限に溜まります。`backups/` プレフィックスに
**60 日で自動削除**のルールを付けてあり、人間が掃除を覚えておく必要がありません。
...
- 読む側(EC2 / Lambda)は IAM で `/minecraft/*` の読み取りだけを許可(04 章)
