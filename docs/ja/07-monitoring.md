...
24 時間稼働だと約 $42/月ペース。そこで**停止まわりは「誰が死んでも別の層が気づく」3 層 + 保険 2 つ**
...
**アラームアクションで直接 EC2 を止めます**(Lambda すら不要)。ただし 50KB/5分は
プレイヤー数を直接見ないヒューリスティックなので、実際の通信量を確認して必要なら調整してください。
接続中でも極端に静かなサーバーは停止される可能性があり、この最後の保険では EC2 内の
正常な保存・バックアップ処理を通りません。`treat_missing_data = "notBreaching"` により、
停止中(メトリクスが来ない)には鳴りません。費用はアラーム 1 本 $0.10/月。
...
「気をつける」だけでなく「失敗を見える化し、影響を抑える」ように設計する。
個人の学習アカウントでも、この姿勢が身につくとより安全に実験できるようになります。
